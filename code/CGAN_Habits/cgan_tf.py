...
# xe: binary cross entropy loss function
...
# Used for Lipschiz regularization
...
class Cgan_tf():
    def __init__(self,preprocessor):
        ...
        D_W3 = tf.Variable(weights_init([D_H_DIM[1], D_H_DIM[2]]))
        D_b3 = tf.Variable(tf.zeros(shape=[D_H_DIM[2]]))
        D_W4 = tf.Variable(weights_init([D_H_DIM[2], 1]))
        ...
        G_W3 = tf.Variable(weights_init([G_H_DIM[1], G_H_DIM[2]]))
        G_b3 = tf.Variable(tf.zeros(shape=[G_H_DIM[2]]))
        G_W4 = tf.Variable(weights_init([G_H_DIM[2], X_dim]))
    ...
    def train(self,paths, preprocessor,loader,distance, datasets):
        mmds_one_itr = []
        sess = tf.Session()
        ...
        for epoch in range(Hyperparam.EPOCHS):
            ...
            # Collect losses for plotting purposes
            losses = np.array([epoch, D_loss_curr, G_loss_curr]).reshape(1, 3)
            self.saved_loss.append(losses)

            if epoch % 100 == 0: #evaluate synthetic data and calculate MMD every now and then
        ...
        """save model"""
        save_path = self.saver.save(sess, paths.current_path + paths.model_fn)
        ...
        saved_loss.to_csv(paths.current_path + paths.loss_fn, index=False)

        """save MMD results"""
        mmds_all_itr = np.vstack(self.mmds_all_itr)
        mmds = pd.DataFrame(mmds_all_itr, columns=datasets.all_short_names)
        mmds.to_csv(paths.current_path + paths.mmd_fn, index=False)
    ...
    def get_intraining_samples(self, sess,preprocessor, X_mb_real, y_mb_real):
        ...
        noise = np.random.normal(0, 1, (len(y_mb_real), Hyperparam.Z_DIM)) if \
            Hyperparam.NOISE == 'normal' else np.random.uniform(-1, 1, (len(y_mb_real), Hyperparam.Z_DIM))
    ...
    def generate_habit(self,paths, input_path,  preprocessor, nof_samples, sampled_labels):
        ...
        """Restore Model"""
        saver.restore(sess, input_path + paths.model_fn)

        """ Generate Habits"""
        trained_output=self.get_trained_habits(sess, preprocessor, nof_samples, sampled_labels)
        trained_output = pd.DataFrame(trained_output, columns=['iteration','name','week','day','hour'])
        trained_output.iloc[:, 2:] = (trained_output.iloc[:, 2:].astype(float)).round(0)
        trained_output.to_csv(paths.current_path + paths.synth_habits_fn, index=False)
    ...
    def get_trained_habits(self, sess, preprocessor, nof_samples=100, sampled_labels=['CDE', 'FRE']):
        stacked_samples = []
        ...
        for i in range(nof_samples):
            ...
            noise = np.random.normal(0, 1, (len(sampled_labels), Hyperparam.Z_DIM)) if Hyperparam.NOISE == 'normal' else np.random.uniform(-1,1, (len(sampled_labels), Hyperparam.Z_DIM))

            gen_signatures = sess.run(self.generator(self.Z,self.y, reuse=True), feed_dict={self.Z: noise, self.y: sampled_labels})

            print('Obtained samples ' + str(i))
